...
def score(query: str, k: int) -> List[Tuple[int, float]]:
    ...
    query_postings = [_loaded_inverted_index.get_postings(t) for t in unique_q]
    query_weights = [q_weights[t] for t in unique_q]
    query_idfs = [idf_cache[t] for t in unique_q]
    
    for doc_int, _ in candidates:
        dot_product = 0.0
        matched = 0
        for i in range(num_unique):
            tf = query_postings[i].get(doc_int, 0)
            if tf > 0:
                dot_product += query_weights[i] * (tf * query_idfs[i])
                matched += 1
