...
def build(index: InvertedIndex) -> None:
    ...
    for term in index.vocabulary:
        ...
        _idf_cache[term] = log((index.N - df + 0.5) / (df + 0.5) + 1)

    avgdl = index.avg_doc_len if index.avg_doc_len > 0 else 1.0
    _doc_len_ratio = [doc_len / avgdl for doc_len in index.doc_len_by_int]
    _doc_norm_cache = {}
...
def _get_doc_norm(b: float) -> List[float]:
    if b not in _doc_norm_cache:
        one_minus_b = 1.0 - b
        _doc_norm_cache[b] = [one_minus_b + b * ratio for ratio in _doc_len_ratio]
...
def score(
    ...
    for term in query_terms:
        ...
        for doc_int, tf in postings.items():
            ...
            denominator = tf + k1 * doc_norm[doc_int]
            term_score = idf * (numerator / denominator)
            scores[doc_int] = scores_get(doc_int, 0.0) + term_score

    return heapq.nlargest(k, scores.items(), key=itemgetter(1))
